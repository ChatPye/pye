...
// Handle chat requests with dev bypass support
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { videoId, question, timestamp, testMode } = body;
    ...
    // Forward to main chat endpoint with dev bypass support
    ...
    console.error('Extension chat API error:', error);
    return NextResponse.json(

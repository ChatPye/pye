    ...
    ;(async () => {
      try {
        const [q, p] = await Promise.all([
          fetch(`/api/questions/top?videoId=${encodeURIComponent(podId)}`),
          fetch(`/api/prompts/top?videoId=${encodeURIComponent(podId)}`),
        ])
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 text-white">
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 p-5 shadow-2xl mb-6">
        <h1 className="text-2xl font-semibold bg-clip-text text-transparent bg-gradient-to-r from-white to-white/70">Pod analytics</h1>
        <p className="text-sm text-white/70">Completion, watch time, prompts, questions, and learners.</p>
      </div>
      {loading ? (
        <div>Loading…</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 shadow">
            <h2 className="font-semibold">Watch time</h2>
            <div className="h-40 mt-3 rounded bg-white/5 flex items-center justify-center text-white/40">Chart (mock)</div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 shadow">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 md:col-span-2 shadow">
            <h2 className="font-semibold">Learners who completed</h2>
            <div className="mt-2 h-40 rounded bg-white/5 flex items-center justify-center text-white/40">Completion table (mock)</div>
            <div className="mt-3 flex justify-end"><button className="px-3 py-2 rounded border border-green-400/40 bg-green-400/15 text-green-100 hover:border-green-300">Issue nano certificates</button></div>
          </div>
        </div>
      )}
    </div>
